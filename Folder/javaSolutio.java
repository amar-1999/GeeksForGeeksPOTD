...
class Tree
{
    ...
    static void inOrderTraversal(Node node, ArrayList<Integer> values) {
        if (node == null) {
            return;
        }
    ...
    public static float findMedian(Node root)
    {
        ...
        if (nodeCount % 2 == 0) {
            int median1 = values.get(nodeCount / 2 - 1);
            int median2 = values.get(nodeCount / 2);
            return (float)(median1 + median2) / 2;
        } else {
            return (float)values.get(nodeCount / 2);
        }
    }
}